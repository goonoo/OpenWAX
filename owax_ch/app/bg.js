...
chrome.browserAction.onClicked.addListener(function() {
	chrome.tabs.getSelected(null, function(tab) {
		if (tab.url.indexOf('http') != 0 && tab.url.indexOf('https')) {
			alert('You can check following schemas only: "http://", "https://"');
			return;
		};
		...
		chrome.tabs.sendMessage(tab.id, {
			action: 'execute_'+ (isIncludeFrame
				? 'with_frames'
				: 'without_frames')}, function(enabled) {
		});
	});
});

chrome.extension.onMessage.addListener(function(request, sender, sendResponse){
	if (request.x) pickColor(request);
});

function pickColor(request) {
	if (request.capture) {
		chrome.tabs.captureVisibleTab(null, {format: 'png'}, function(dataUrl) {
			img.onload = function() {pickColor2(request)};
